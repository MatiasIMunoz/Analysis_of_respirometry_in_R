...
stop_flow_analysis <- function(df,
  ...
  results <- data.frame(
    Repetition = integer(),
    Markers = integer(),
    Enclosure_time_min = integer(),
    Start_sample_sec = numeric(),
    End_sample_sec = numeric(),
  ...
  for (i in 1:nrow(markers_df_subset)) {
    ...
    results <- rbind(results, data.frame(
      ...
      Enclosure_time_min = (enclosure_time*nfrogs)/60,
      Start_sample_sec = begin_end[[1]],
      End_sample_sec = begin_end[[2]],
